...
namespace ZeroElectric.Fenestra
{
	public class SetupBuilder
	{
		...
		public void BuildEXE(MPackManifest header, bool buildAsPortable = false)
		{
			string aepkgOutputPath = Path.Combine(FileSystemHelper.Output, $"{header.output.pkgName}-{header.pkgVer}.aepkg");

			Log.Information("Preparing files for setup.exe compilation...");

			foreach (string file in Directory.GetFiles(FileSystemHelper.BaseDirectory))
			{
				...
				if (ext == ".exe" || ext == ".dll" || ext == ".aepkg" || ext == ".aepak")
				{
					...
					if (ext == ".exe")
					{
						tempFile = Path.Combine(FileSystemHelper.TempSetup, $"{header.output.installerName}.exe");
						Log.Debug("An EXE was found: {name}, and was copied into: {des}", fileInfo.Name, tempFile);
					}
					else
					{
						tempFile = Path.Combine(FileSystemHelper.TempSetup, fileInfo.Name);
			...
			foreach (string file in Directory.GetFiles(FileSystemHelper.TempSetup))
			{
				AddFile(file);
			}
			...
			string exePath = Path.Combine(FileSystemHelper.OutputBin, $"{header.output.installerName}.exe");
	...
	{
		public static string GetInstallCode(MPackManifest header, bool buildAsPortable = false)
		{
			string folderName = header.output.shortcutName;
			string exeName = $"{header.output.launcherName}.exe";
			...
			string srcPath = Path.Combine(FileSystemHelper.Temp, "src");

			Directory.CreateDirectory(srcPath);

			string aieCS = Path.Combine(srcPath, "aei.cs");
